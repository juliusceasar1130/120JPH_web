<template>
    ...
    <el-collapse-item name="2">
      ...
      <el-card style="margin: 5px 0px">
        ...
        <el-pagination v-model:current-page="pageNo" v-model:page-size="pageSize" :page-sizes="[10, 20, 30, 50]"
          ...
          @size-change="getLine3ListByPage(1)" @current-change="getLine3ListByPage" />
      </el-card>
    </el-collapse-item>
  ...
  <!--弹出图片部分-->
  <el-dialog v-model="visible" draggable="true" style="width: 1100px; height: 550px">
    ...
    <el-div style="width: 1000px; height: 500px">
      <el-carousel indicator-position="outside" arrow="always" style="width: 1050px; height: 450px">
        <el-carousel-item v-for="item in dialogInfo.imgList" :key="item.name" style="width: 1090px; height: 450px">
          <h3 text="2xl" justify="center">{{ item.name }}</h3>
          <hr />
          <div>
            <el-image style="width: 500px; height: 300px" fit="cover" :src="item.url" :preview-src-list="[item.url]"
  ...
  <el-dialog v-model="dialogImportVisible" title="导入" width="30%">
    <el-form label-width="120px">
      <el-form-item label="门槛数据">
        <el-upload class="upload-demo" :action="setting.RP3_importData_path" :on-success="onUploadSuccess">
...
const exportData = () => {
  console.log('导出数据')
  reqRp3ExcelDataExport().then((res) => {
    const blob = new Blob([res], { type: 'application/vnd.ms-excel' })
    const link = document.createElement('a')
    link.href = window.URL.createObjectURL(blob)
    link.download = '门槛3站导出数据.xls'
    link.click()
  })
}
//导入按钮
